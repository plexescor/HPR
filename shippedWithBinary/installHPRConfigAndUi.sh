...
if [[ -f "$CONFIG_DIR/projectAliases.csv" ]]; then
    read -p ">> projectAliases.csv already exists. Overwrite? (y/N): " confirm_aliases
    if [[ "$confirm_aliases" == "y" || "$confirm_aliases" == "Y" ]]; then
