									...
									const std::map<std::string, std::string> &headers)
{
	...
	if (AppState::extManager)
	{
		...
		auto res = AppState::extManager->dispatchOverride(
			"httpGet", {CppValue(CppValue::Type::String, host), CppValue(CppValue::Type::String, path),
						CppValue(CppValue::Type::Bool, secure), headersVal});
...
#ifdef _WIN32
	HINTERNET hSession =
		WinHttpOpen(L"HPR/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
	if (!hSession)
		return {"", 0};
	...
	HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", wPath.c_str(), NULL, WINHTTP_NO_REFERER,
	...
	for (const auto &[k, v] : headers)
	{
		...
		WinHttpAddRequestHeaders(hRequest, wHeaderLine.c_str(), -1L,
...
std::pair<std::string, int> httpPost(const std::string &host, const std::string &path, const std::string &body,
									 bool secure, const std::map<std::string, std::string> &headers)
{
	if (!AppState::configManager.getConfig<bool>("allow-network-activity", true))
	{
		return {"", 0};
	...
	if (AppState::extManager)
	{
		...
		auto res = AppState::extManager->dispatchOverride(
			"httpPost", {CppValue(CppValue::Type::String, host), CppValue(CppValue::Type::String, path),
						 CppValue(CppValue::Type::String, body), CppValue(CppValue::Type::Bool, secure), headersVal});
		if (res.has_value() && res->type == CppValue::Type::Struct)
		{
			std::string resBody = "";
			int status = 200;
	...
	if (WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, (LPVOID)body.c_str(), body.size(), body.size(),
						   0) &&
		WinHttpReceiveResponse(hRequest, NULL))
	{
		// Query HTTP Status Code
	...
	if (curl)
	{
		...
		{
			std::string lowerK = k;
			std::transform(lowerK.begin(), lowerK.end(), lowerK.begin(),
									...
									bool secure, const std::map<std::string, std::string> &headers)
{
	...
	if (AppState::extManager)
	{
		...
		auto res = AppState::extManager->dispatchOverride(
			"httpPut", {CppValue(CppValue::Type::String, host), CppValue(CppValue::Type::String, path),
						CppValue(CppValue::Type::String, body), CppValue(CppValue::Type::Bool, secure), headersVal});
		if (res.has_value() && res->type == CppValue::Type::Struct)
		{
			std::string resBody = "";
	...
	HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"PUT", wPath.c_str(), NULL, WINHTTP_NO_REFERER,
											WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
	if (!hRequest)
	{
		WinHttpCloseHandle(hConnect);
...
std::pair<std::string, int> httpDelete(const std::string &host, const std::string &path, bool secure,
									   const std::map<std::string, std::string> &headers)
{
	if (!AppState::configManager.getConfig<bool>("allow-network-activity", true))
	{
	...
	if (AppState::extManager)
	{
		...
		auto res = AppState::extManager->dispatchOverride(
			"httpDelete", {CppValue(CppValue::Type::String, host), CppValue(CppValue::Type::String, path),
						   CppValue(CppValue::Type::Bool, secure), headersVal});
		if (res.has_value() && res->type == CppValue::Type::Struct)
		{
			std::string resBody = "";
	...
	HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"DELETE", wPath.c_str(), NULL, WINHTTP_NO_REFERER,
											WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
	if (!hRequest)
	{
		WinHttpCloseHandle(hConnect);
...
void handleClient(SOCKET clientSocket, sol::function handler, sol::state &lua)
{
	...
	std::string request_line =
		(first_line_end != std::string::npos) ? headers_part.substr(0, first_line_end) : headers_part;
	...
	while (pos != std::string::npos && pos < headers_part.length())
	{
		...
		std::string line =
			(line_end != std::string::npos) ? headers_part.substr(pos, line_end - pos) : headers_part.substr(pos);
