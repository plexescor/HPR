...
sol::object parseValue(sol::state &lua, const std::string &str, size_t &pos, int depth = 0)
{
	if (depth > 500)
	{
		...
		std::string warningMsg = "\n[HPR SECURITY ERROR] Active Lua extension [" + extName +
								 ...
								 "by parsing a deeply nested JSON payload "
								 "(exceeded max recursion depth of 500).\n";
		std::cerr << warningMsg;
...
static std::string toJSON_Internal(const sol::object &obj, std::unordered_set<const void *> &visited)
{
	...
	else if (obj.is<sol::table>())
	{
		...
		if (visited.find(tabPtr) != visited.end())
		{
			...
			std::string warningMsg = "\n[HPR SECURITY ERROR] Active Lua extension [" + extName +
									 "] attempted to trigger a stack overflow / crash "
									 "by serializing a cyclic self-referential table (circular "
									 "reference "
									 "detected at address " +
									 std::to_string(reinterpret_cast<uintptr_t>(tabPtr)) +
									 "). Serializing as null to prevent SegFault.\n";
			std::cerr << warningMsg;
			Logger::log(warningMsg);
			return "null";
		}
		...
		// Determine if this is a Lua array (consecutive integer keys starting
		// at 1) or an object (string-keyed table)
		bool isArray = false;
		size_t arrayLen = 0;
