...
std::string normalizeKey(const std::string &key)
{
	...
	if (key == "\t" || key == "tab")
		return "tab";
	if (key == "\u0008" || key == "\b" || key == "backspace")
		return "backspace";
	if (key == "\u007f" || key == "delete" || key == "del")
		return "delete";
	if (key == "\u0010" || key == "shift")
		return "shift";
	if (key == "\u0015" || key == "control" || key == "ctrl")
		return "control";
	if (key == "\u0016" || key == "alt")
		return "alt";
	return key;
							 ...
							 HPRInterpreter *interpreter)
{
	...
	ui->on_loadHistoricalData_Singular(
		[](slint::SharedString dateFromUi)
		{
	...
	ui->on_loadHistoricalData_Number(
		[](int days, slint::SharedString mode)
		{
			std::string modeStr = std::string(mode);
			// Emit the signal that we need to load data
			EventHub::emit(Event::LOAD_DATABASE_NUMBER, DatabaseDate_Number{days, modeStr});
		});

	ui->on_loadHistoricalData_Range(
		[](slint::SharedString dateFrom, slint::SharedString dateTo, slint::SharedString mode)
		{
			std::string dateFromStr = std::string(dateFrom);
			std::string dateToStr = std::string(dateTo);
			std::string modeStr = std::string(mode);
			// Emit the signal that we need to load data
			EventHub::emit(Event::LOAD_DATABASE_RANGE, DatabaseDate_Range{dateFromStr, dateToStr, modeStr});
		});
	...
	ui->on_tabViewClicked([this]() { this->tabViewClicked(); });

	ui->on_siteViewClicked([this]() { this->siteViewClicked(); });

	ui->on_rawViewClicked([this]() { this->rawViewClicked(); });

	ui->on_filterViewClicked([this]() { this->filterViewClicked(); });

	if (extManager)
	{
		ui->on_refreshExtensions([this]() { std::thread([this]() { extManager->refresh(); }).detach(); });
		ui->on_disableExtension(
	...
	ui->on_setLimit(
		[](slint::SharedString appName, int minutes)
		{
	...
	ui->on_loadInsights(
		[this]()
		{
			...
			std::thread([days]() { EventHub::emit(Event::LOAD_PATTERNS_DATA, PatternDataRequest{days}); }).detach();
		});
...
// Open webbrowser
#ifdef _WIN32
			ShellExecuteA(nullptr, "open", "https://github.com/plexescor/HPR/releases", nullptr, nullptr,
						  SW_SHOWNORMAL);
#else
...
// Open webbrowser
	...
	ui->on_miscKeyPressed_S(
		[this](slint::SharedString key)
		{
	...
	ui->on_sendFeedback(
		[weak_ui](slint::SharedString email, slint::SharedString message)
		{
			std::thread(
				[weak_ui, emailStr = std::string(email), msgStr = std::string(message)]()
				{
					...
					std::string body = "{\"email\":\"" + jsonEscape(emailStr) + "\",\"message\":\"" +
									   jsonEscape(msgStr) + "\",\"timestamp\":" + std::to_string(ts) +
									   ",\"version\":\"" + jsonEscape(AppState::APP_VERSION) + "\",\"os\":\"" +
									   jsonEscape(osStr) + "\"";
					if (!compStr.empty())
					{
							 ...
							 ExtensionManager *extMgr, HPRInterpreter *interpreter)
{
	...
	ui->set_callback("loadHistoricalData_Singular",
					 [](auto args) -> slint::interpreter::Value
					 {
						 ...
						 if (args.size() > 0)
						 {
							 // Safely convert the interpreter value to an optional
							 // SharedString
							 auto opt_str = args[0].to_string();

							 if (opt_str.has_value())
							 {
								 // Dereference the optional to get the SharedString,
	...
	ui->set_callback("loadHistoricalData_Number",
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 1)
						 {
	...
	ui->set_callback("loadHistoricalData_Range",
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 2)
						 {
							 ...
							 if (opt_dateFrom.has_value() && opt_dateTo.has_value() && opt_mode.has_value())
							 {
								 ...
								 EventHub::emit(Event::LOAD_DATABASE_RANGE,
	...
	ui->set_callback("tabViewClicked",
					 [this](auto args) -> slint::interpreter::Value
					 {
						 // No need for event hub
						 this->tabViewClicked();
	...
	ui->set_callback("siteViewClicked",
					 [this](auto args) -> slint::interpreter::Value
					 {
						 // No need for event hub
	...
	ui->set_callback("rawViewClicked",
					 [this](auto args) -> slint::interpreter::Value
					 {
						 // No need for event hub
						 this->rawViewClicked();
	...
	ui->set_callback("filterViewClicked",
					 [this](auto args) -> slint::interpreter::Value
					 {
						 // No need for event hub
	...
	if (extManager)
	{
		ui->set_callback("refreshExtensions",
						 [this](auto args) -> slint::interpreter::Value
						 {
							 std::thread([this]() { extManager->refresh(); }).detach();
							 return slint::interpreter::Value();
						 });
					 ...
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 1)
						 {
							 ...
							 if (opt_name.has_value() && opt_mins.has_value())
							 {
								 ...
								 if (AppState::limitsManager)
								 {
									 AppState::limitsManager->setLimit(std::string(opt_name.value()),
																	   (int)opt_mins.value());
								 }
							 }
						 }
					 ...
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 1)
						 {
							 ...
							 if (opt_name.has_value() && opt_mins.has_value())
							 {
								 ...
								 if (AppState::limitsManager)
								 {
									 AppState::limitsManager->setGoal(std::string(opt_name.value()),
																	  (int)opt_mins.value());
								 }
							 }
						 }
...
// open browser
	...
	ui->set_callback("openIssues",
					 [this](auto args) -> slint::interpreter::Value
					 {
// open browser
#ifdef _WIN32
						 ShellExecuteA(nullptr, "open", "https://github.com/plexescor/HPR/issues", nullptr, nullptr,
									   SW_SHOWNORMAL);
#else
	...
	ui->set_callback("minimize_window",
					 [weak_ui](auto args) -> slint::interpreter::Value
					 {
						 if (auto handle = weak_ui.lock())
						 {
							 (*handle)->hide();
	...
	ui->set_callback("miscKeyPressed_S",
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 0)
						 {
							 ...
							 if (opt_key.has_value())
							 {
								 ...
								 std::string normalized = normalizeKey(k);
								 Logger::log("[UIEventBridge] miscKeyPressed_S (interpreted): '" + k + "' -> '" +
											 normalized + "'");
								 EventHub::emit("MISC_KEY_PRESSED", CppValue(CppValue::Type::String, normalized));
							 }
						 }
	...
	ui->set_callback("miscKeyReleased_S",
					 [](auto args) -> slint::interpreter::Value
					 {
						 if (args.size() > 0)
						 {
							 ...
							 if (opt_key.has_value())
							 {
								 std::string k = std::string(opt_key.value());
								 std::string normalized = normalizeKey(k);
								 Logger::log("[UIEventBridge] miscKeyReleased_S (interpreted): '" + k + "' -> '" +
											 normalized + "'");
								 EventHub::emit("MISC_KEY_RELEASED", CppValue(CppValue::Type::String, normalized));
							 }
						 }
				...
				if (opt_email.has_value() && opt_msg.has_value())
				{
					...
					std::thread(
						[weak_ui, email, msg]()
						{
							...
							std::string body = "{\"email\":\"" + jsonEscape(email) + "\",\"message\":\"" +
											   jsonEscape(msg) + "\",\"timestamp\":" + std::to_string(ts) +
											   ",\"version\":\"" + jsonEscape(AppState::APP_VERSION) + "\",\"os\":\"" +
											   jsonEscape(osStr) + "\"";
							if (!compStr.empty())
							{
							...
							slint::invoke_from_event_loop(
								[weak_ui, success, errMsg]()
								{
									if (auto handle = weak_ui.lock())
									{
										(*handle)->set_property("is-sending-feedback",
																slint::interpreter::Value(false));
										(*handle)->set_property("feedbackSuccess", slint::interpreter::Value(success));
										(*handle)->set_property("feedbackErrorMsg",
																slint::interpreter::Value(slint::SharedString(errMsg)));
										(*handle)->set_property("showFeedbackResultPrompt",
																slint::interpreter::Value(true));
										if (success)
										{
											(*handle)->set_property("feedbackMessage",
																	slint::interpreter::Value(slint::SharedString("")));
										}
									}
								});
						})
...
void UiEventBridge::init()
{
	...
	loadDbSingularId = EventHub::connect(Event::HISTORY_LOADED_SINGULAR,
										 [this](EventData data) { this->showHistoricalDataSingular(); });

	loadDbNumberId =
		EventHub::connect(Event::HISTORY_LOADED_NUMBER, [this](EventData data) { this->showHistoricalDataNumber(); });
...
void UiEventBridge::showLiveData()
{
	EventHub::emit(Event::LOAD_LIVE_DATA); // Tell everyone we need live data, so they can
										   // prepare it before we switch the view
