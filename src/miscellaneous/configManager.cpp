...
template <typename T> T ConfigManager::getConfig(const std::string requestedParam, const T &defaultValue)
{
	for (const auto &[param, value] : config)
	{
...
template <typename T> void ConfigManager::setConfig(const std::string paramName, const T &value)
{
	std::string valueStr;
	if constexpr (std::is_same_v<T, bool>)
	{
	...
	if (paramName == "autostart" || paramName == "headless-mode")
	{
		...
		{
			if (param == "autostart")
				autostartVal = (val == "true");
			if (param == "headless-mode")
				headlessVal = (val == "true");
		}
