...
void AliasManager::loadAliases()
{
	...
	if (!file.is_open())
	{
		std::cerr << "Warning: aliases.csv not found at " << filePath.string() << ". Using raw names.\n";
...
void AliasManager::loadAliases_Tab()
{
	...
	if (!file.is_open())
	{
		std::cerr << "Warning: tabAliases.csv not found at " << filePath_Tab.string() << ". Using raw names.\n";
...
void AliasManager::loadAliases_Project()
{
	...
	if (!file.is_open())
	{
		...
		Logger::log("Warning: projectAliases.csv not found at " + filePath_Project.string() +
					". will use raw but parsed names.");
		return;
	}
