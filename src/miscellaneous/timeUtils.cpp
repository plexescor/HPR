...
std::string extractMMYY_from_DDMMYY(const std::string &dateStr)
{
	try
	{
		if (dateStr.size() != 8 || !std::isdigit((unsigned char)dateStr[0]) ||
			!std::isdigit((unsigned char)dateStr[1]) || dateStr[2] != '-' || !std::isdigit((unsigned char)dateStr[3]) ||
			!std::isdigit((unsigned char)dateStr[4]) || dateStr[5] != '-' || !std::isdigit((unsigned char)dateStr[6]) ||
