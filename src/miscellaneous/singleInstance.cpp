...
SingleInstance::~SingleInstance() { shutdown(); }
...
void SingleInstance::onShow(std::function<void()> callback) { showCallback = callback; }
...
void SingleInstance::runServer()
{
	...
	while (running)
	{
		HANDLE hPipe = CreateNamedPipeW(pipeName.c_str(), PIPE_ACCESS_DUPLEX,
