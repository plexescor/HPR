...
void NowPlayingManager::init()
{
    ...
    std::thread([]() {
        // Wait 5 seconds after startup for database initialization to settle
...
void NowPlayingManager::runCycle()
{
    try
    {
        ...
        // Only upload to database if the title has changed to minimize DB writes
        if (cleanTitle != lastUploadedTitle)
        {
            ...
            // Step 1: write the secret node (rules allow this when value === password)
            ...
            {
                // Step 2: write data (rules check sibling secret node === password)
                std::string dataBody = "{\"title\":\"" + escapeJsonString(cleanTitle) +
                                       "\",\"url\":\"" + escapeJsonString(videoUrl) + "\"}";
        ...
        // All clients (including the dev client) perform this so the About view is updated
...
std::string NowPlayingManager::extractJsonValue(const std::string& json, const std::string& key)
{
    ...
    for (size_t i = 0; i < value.length(); ++i)
    {
        if (value[i] == '\\' && i + 1 < value.length())
        {
            char next = value[i + 1];
            if (next == '"') cleanValue += '"';
            else if (next == '\\') cleanValue += '\\';
            else if (next == 'n') cleanValue += '\n';
            else if (next == 'r') cleanValue += '\r';
            else if (next == 't') cleanValue += '\t';
            else cleanValue += next;
            i++;
        }
        else
        {
