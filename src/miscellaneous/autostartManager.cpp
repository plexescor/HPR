...
void AutostartManager::setEnabled(bool enable, bool headless) {
    ...
    if (enable) {
        ...
        // Note: headless-mode is handled via config.csv directly, so no command line argument is appended
        RegSetValueExW(
            hKey,
            L"HPR",
            0,
    ...
    if (enable) {
        ...
        if (std::filesystem::exists(sourceFile)) {
            ...
            // Fallback if launcher desktop file is not found (e.g. running from build dir)
