...
{
	{
		...
		else
		{
			...
			AppState::state.limitTimeBase[appName] =
				AppState::state.timeLog_PerApp.count(appName) ? AppState::state.timeLog_PerApp[appName] : 0;
		}
	}
		...
		for (const auto &[appName, minutes] : activeLimits)
		{
			...
			if (currentUsage.count(appName))
			{
				...
				if (doKill)
				{
					...
					{
						showNotification("HPR Termination", aliasedName + " has exceeded its limit by 5%. "
																		  "Terminating process!");

						// Check global kill cooldown
						auto now = std::chrono::steady_clock::now();
						auto timeSinceLastKill =
							std::chrono::duration_cast<std::chrono::milliseconds>(now - lastGlobalKillTime).count();
						...
						if (timeSinceLastKill >= killCooldown)
						{
							...
							// otherwise it kills them even if they run in
							// background
				...
				if (killSent[appName])
				{
					if (AppState::configManager.getConfig("kill-apps", true))
					{
						// Check global kill cooldown before repeating kill
						// command
						auto now = std::chrono::steady_clock::now();
						auto timeSinceLastKill =
							std::chrono::duration_cast<std::chrono::milliseconds>(now - lastGlobalKillTime).count();
