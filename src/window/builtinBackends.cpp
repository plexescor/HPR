...
std::string buildScript(const std::string &dbusAddr, const std::string &action)
{
	return R"(
                function output_result(message) {
                    ...
                    callDBus(")" +
		   dbusAddr + R"(", "/", "", "result", message.toString());
                }
                function output_error(message) {
                    callDBus(")" +
		   dbusAddr + R"(", "/", "", "error", message.toString());
                }
...
std::string kwinScriptGetResult(const std::string &marker, const std::string &action)
{
	...
	// Use self_conn's unique name as the dbus addr (like kdotool's
	// self_conn.unique_name())
	...
	dbus_message_append_args(msg, DBUS_TYPE_STRING, &pathCStr, DBUS_TYPE_STRING, &nameCStr, DBUS_TYPE_INVALID);
...
void registerBuiltinBackends()
{
	registerBackend(
		{"GNOME", [](const std::string &desktopEnvironment) { return desktopEnvironment.contains("GNOME"); },
		 ...
		 []()
		 {
			 std::string extensionDirectory = std::string(getenv("HOME")) + "/.local/share/gnome-shell/extensions/"
																			"lol-another-window-extension@plexescor";

			 bool extensionFilesExist = std::filesystem::exists(extensionDirectory);

			 std::string extensionCheckCommand = "gdbus call --session "
			 ...
			 std::string extensionCheckResult = runSystemCommand_UNSAFE(extensionCheckCommand);

			 Logger::log("[HPR] GNOME extension check: " + extensionCheckResult);

			 bool extensionWorking = extensionCheckResult.contains("('");
		 ...
		 []() -> std::string
		 {
			 std::string command = "gdbus call --session --dest org.gnome.Shell --object-path "
								   "/org/gnome/Shell/Extensions/LolAnotherWindowExtension "
								   "--method "
								   "org.gnome.Shell.Extensions.LolAnotherWindowExtension."
								   "FocusClass";
	...
	registerBackend({"KDE", [](const std::string &env) { return env.contains("KDE"); },
					 ...
					 []() -> std::string { return NiceKDE::kdeGetActiveWindowPid(); }, true
	...
	registerBackend({"Cinnamon",
					 ...
					 []() -> std::string
					 {
						 std::string cmd = "gdbus call --session --dest org.Cinnamon --object-path "
										   "/org/Cinnamon --method org.Cinnamon.Eval "
