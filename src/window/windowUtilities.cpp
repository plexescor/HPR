...
const std::vector<std::string> blockedCommands = {
	...
	"| tee /sys",
	"mv ",
	"mv\t",		// prevent moving/renaming system files
...
std::string runSystemCommandWindows(const std::string &command, int &exitCode)
{
	...
	BOOL bSuccess = CreateProcessA(NULL, cmdLineBuf.data(), NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL,
...
std::string runSystemCommand(std::string &command)
{
	...
	if (exitCode != 0)
	{
		...
		Logger::log("[HPR] Running the command failed! Exit Code: " + std::to_string(exitCode) +
					" for command: " + command);
	}
...
std::string runSystemCommand_UNSAFE(std::string &command)
{
	...
	if (exitCode != 0)
	{
		//   std::cerr << "Running the command failed! Exit Code: " <<
		//   WEXITSTATUS(exitCode) << std::endl; Logger::log("[HPR] Running the
		//   command failed! Exit Code: " +
		//   std::to_string(WEXITSTATUS(exitCode)) + " for command: " +
		//   command);
	}
	return output;
	...
	if (exitCode != 0)
	{
		...
		//   << std::endl; Logger::log("[HPR] Running the command failed! Exit
		//   Code: "
		//   + std::to_string(exitCode) + " for command: " + command);
	}
...
void showNotification(const std::string &title, const std::string &msg)
{
	...
	if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - startTime).count() <
		1200)
		return;
	...
	if (AppState::extManager)
	{
		auto res = AppState::extManager->dispatchOverride(
			"showNotification", {CppValue(CppValue::Type::String, title), CppValue(CppValue::Type::String, msg)});
		if (res.has_value())
		{
			return;
		}
...
std::string getCpuUsage()
{
	...
	if (lastCloseParen == std::string::npos || lastCloseParen + 2 >= line.size())
		return "0.00%";
	...
	if (!(iss >> state >> ppid >> pgrp >> session >> tty_nr >> tpgid >> flags >> minflt >> cminflt >> majflt >>
		  cmajflt >> utime >> stime))
	{
		return "0.00%";
	}
	...
	if (elapsed == 0 || clockTicks <= 0 || numCores <= 0)
		return "0.00%";
	...
	kern_return_t kr =
		task_info(mach_task_self(), TASK_THREAD_TIMES_INFO, (task_info_t)&threadTimes, &threadTimesCount);
	if (kr != KERN_SUCCESS)
		return "0.00%";
