...
void TimelineManager::updateTimeline(int presetHours, int startHour, int endHour)
{
	...
	std::sort(transitions.begin(), transitions.end(),
			  [](const Transition &a, const Transition &b) { return a.ts < b.ts; });
	...
	auto processSegment = [&](const std::string &appName, uint64_t segStart, uint64_t segEnd)
	{
		...
		filteredSegments.push_back({x, width, aliasedName, durationStr, rangeStr});
	};
...
void TimelineManager::threadLoop()
{
	while (running)
	{
		...
		// regular rebuild of the timeline. Parameters will be updated directly
		// by UiModelManager reading from the UI properties.
