...
DatabaseManager::DatabaseManager()
{
											   ...
											   [this](EventData data)
											   {
												   ...
												   if (std::holds_alternative<DatabaseDate_Singular>(data))
												   {
													   std::string requestedDate =
														   std::get<DatabaseDate_Singular>(data).date;
													   this->loadDb_Singular(requestedDate);
												   }
											   });

	number_DbLoadEventId = EventHub::connect(Event::LOAD_DATABASE_NUMBER,
											 [this](EventData data)
											 {
	...
	range_DbLoadEventId = EventHub::connect(Event::LOAD_DATABASE_RANGE,
											[this](EventData data)
											{
	...
	patterns_DbLoadEventId = EventHub::connect(Event::LOAD_PATTERNS_DATA,
											   [this](EventData data)
											   {
...
void DatabaseManager::initDatabase(bool copyData)
{
	...
	*db << "create table if not exists app_usage ("
		   "   name text unique,"
		   "   duration int"
		   ");";

	*db << "create table if not exists tab_usage("
		   "    name text unique,"
		   "    duration int"
		   ");";

	*db << "create table if not exists project_usage("
		   "    name text unique,"
		   "    duration int"
		   ");";

	*db << "create table if not exists switch_history ("
		   "   fromWindow text,"
		   "   toWindow text,"
		   "   timeStamp int unique"
		   ");";

	*db << "create table if not exists app_limits ("
		   "   name text unique,"
		   "   minutes int"
		   ");";

	*db << "create table if not exists app_goals ("
		   "   name text unique,"
		   "   minutes int"
		   ");";
...
bool DatabaseManager::loadStateFromDB()
{
	try
	{
		...
		*db << "select name, duration from app_usage;" >> [](std::string name, uint64_t duration)
		{ AppState::state.timeLog_PerApp[name] += duration; };

		*db << "select name, duration from tab_usage;" >> [](std::string name, uint64_t duration)
		{ AppState::state.timeLog_PerTab[name] += duration; };

		*db << "select name, duration from project_usage;" >> [](std::string name, uint64_t duration)
		...
		*db << "select name, minutes from app_limits;" >> [](std::string name, int minutes)
		{ AppState::state.appLimits[name] = minutes; };

		*db << "select name, minutes from app_goals;" >> [](std::string name, int minutes)
		{ AppState::state.appGoals[name] = minutes; };

		*db << "select name, base_ms from limit_bases;" >> [](std::string name, uint64_t base)
		{ AppState::state.limitTimeBase[name] = base; };

		*db << "select name, base_ms from goal_bases;" >> [](std::string name, uint64_t base)
...

		{
			...
			*db << "BEGIN;";
			...
			for (const auto &[k, v] : timeLog_PerApp_D)
				*db << "insert or replace into app_usage (name,duration) "
					   "values (?,?);"
					<< k << v;

			for (const auto &[k, v] : timeLog_PerTab_D)
				*db << "insert or replace into tab_usage (name,duration) "
					   "values (?,?);"
					<< k << v;

			for (const auto &[k, v] : timeLog_PerProject_D)
				*db << "insert or replace into project_usage (name,duration) "
					   "values "
					   "(?,?);"
					<< k << v;
			...
			for (const auto &[k, v] : switchHistory_D)
			{
				...
				for (const auto &timestamp : v)
					*db << "insert or ignore into switch_history "
						   "(fromWindow,toWindow,timeStamp) values (?,?,?);"
			...
			*db << "delete from app_limits;";
			for (const auto &[k, v] : appLimits_D)
				if (v > 0)
					*db << "insert or replace into app_limits (name,minutes) "
						   "values "
						   "(?,?);"
						<< k << v;
			...
			*db << "delete from app_goals;";
			for (const auto &[k, v] : appGoals_D)
				if (v > 0)
					*db << "insert or replace into app_goals (name,minutes) "
						   "values (?,?);"
						<< k << v;

			*db << "delete from limit_bases;";
			for (const auto &[k, v] : limitTimeBase_D)
				*db << "insert or replace into limit_bases (name,base_ms) "
					   "values (?,?);"
					<< k << v;

			*db << "delete from goal_bases;";
			for (const auto &[k, v] : goalTimeBase_D)
				*db << "insert or replace into goal_bases (name,base_ms) "
					   "values (?,?);"
					<< k << v;
...
std::string DatabaseManager::getDbPathForDate(const std::string &date)
{
	...
	path = home;
	path /= std::filesystem::path(".local/share/HPR/HPR_DB") / extractMMYY_from_DDMMYY(date) / (date + ".db");
#endif
	return path.string();
}
		...
		std::async(std::launch::async,
				   [this, requestedDate]()
				   {
					   try
					   {
						   ...
						   // DEVELOPER NOTES:
						   // As its the free version, i therefore only allow loading of
						   // the db files created this month of course you can modify the
						   // source code and give everyone the version that handles the
						   // month also and i will not do anything, :)

						   std::filesystem::path path = getDbPathForDate(requestedDate);
						   loadedHistDbPath = path;

						   // Check if shit even exists
						   if (!std::filesystem::exists(path))
						   {
							   EventHub::emit(Event::APP_ERROR, ErrorGui{"File not found! " + path.string()});
							   std::cerr << "Historical file not found: " << path << std::endl;
							   Logger::log("[HPR] Historical file not found: " + path.string());
							   return;
						   }

						   // Load a new db
						   sqlite::database histDb(path.string());

						   // Force so incomplete .db files possibly due to a crash are
						   // fully resolved by a WAL checkpoint
						   ...
						   histDb << "select name, duration from app_usage;" >>
							   [&results](std::string name, uint64_t duration) { results[name] = duration; };

						   histDb << "select name, duration from tab_usage;" >>
							   [&results_Tab](std::string name, uint64_t duration) { results_Tab[name] = duration; };
						   ...
						   histDb << "select fromWindow, toWindow, timeStamp from "
									 "switch_history;" >>
	...
	historyLoadTask_Number = std::async(
		...
		[this, days, mode]()
		{
			...
			if (mode == "total")
			{
				...
				for (int i = 0; i < days; ++i)
				{
					workers.emplace_back(
						[this, i, &fileNames, &path, activeFileName]()
						{
							try
							{
								...
								else
								{
									std::filesystem::path fullPath =
										path / extractMMYY_from_DDMMYY(fileNames[i]) / (fileNames[i] + ".db");

									if (!std::filesystem::exists(fullPath))
									{
										std::cerr << "Historical file not found: " << fullPath << std::endl;
										Logger::log("[HPR] Historical file not "
													"found: " +
													fullPath.string());
										return;
									}
									...
									// load from db to the map
									numDb << "select name, duration from "
											 "app_usage;" >>
										[&results](std::string name, uint64_t duration) { results[name] = duration; };
									...
									numDb << "select fromWindow, toWindow, "
											 "timeStamp from "
											 "switch_history;" >>
										[&results_Switch](std::string from, std::string to, long long ts)
									{ results_Switch[{from, to}].push_back((uint64_t)ts); };
...

								{
									...
									for (const auto &[key, value] : results_Switch)
									{
										AppState::historicalData_Full_State.switchHistory[key].insert(
											AppState::historicalData_Full_State.switchHistory[key].end(), value.begin(),
											value.end());
									}
			...
			if (mode == "average")
			{
				...
				{
					workers.emplace_back(
						[i, &fileNames, &path, &allApp, &allTab, &allProject, &allSwitches, &loaded, activeFileName]()
						{
							try
							{
								if (fileNames[i] + ".db" == activeFileName)
								...
								else
								{
									...
									numDb << "select name, duration from "
											 "app_usage;" >>
										[&, i](std::string name, uint64_t duration) { allApp[i][name] = duration; };

									numDb << "select name, duration from "
											 "tab_usage;" >>
										[&, i](std::string name, uint64_t duration) { allTab[i][name] = duration; };

									numDb << "select name, duration from "
											 "project_usage;" >>
										[&, i](std::string name, uint64_t duration) { allProject[i][name] = duration; };
				...
				for (int i = 0; i < days; ++i)
				{
					if (!loaded[i])
						continue;
					for (const auto &[k, v] : allApp[i])
					{
						totalApp[k] += v;
						countApp[k]++;
					}
					for (const auto &[k, v] : allTab[i])
					{
						totalTab[k] += v;
						countTab[k]++;
					}
					for (const auto &[k, v] : allProject[i])
					{
						totalProject[k] += v;
						countProject[k]++;
					}
	...
	historyLoadTask_Range = std::async(
		...
		[this, dateFrom, dateTo, mode]()
		{
			...
			if (timeStampFrom == 0 || timeStampTo == 0)
			{
				...
				Logger::log("[HPR] Invalid date format! Dates should be in "
							"DDMMYY format.");
				return;
			}
...
// Synchronous: loads N days of per-day data into PatternAnalyzer for advanced
// insight computation. Called directly on the EventHub thread — no async needed
// because the UI just reads the output strings.
void DatabaseManager::loadPatternsData(int days)
{
	...
	for (int i = 0; i < days; ++i)
	{
		...
		{
			std::filesystem::path fullPath = basePath / extractMMYY_from_DDMMYY(dateStr) / (dateStr + ".db");
			if (!std::filesystem::exists(fullPath))
			{
				// Missing day — skip silently (user may not have data for every
				// day)
				continue;
			}
			...
			try
			{
				...
				dayDb << "select name, duration from app_usage;" >> [&dayData](std::string name, uint64_t duration)
				{ dayData.timePerApp[name] = duration; };

				dayDb << "select fromWindow, toWindow, timeStamp from "
						 "switch_history;" >>
			...
			catch (const std::exception &e)
			{
				...
				Logger::log("[loadPatternsData] Failed to load " + fullPath.string() + ": " + std::string(e.what()));
				continue;
			}
		}
...
void DatabaseManager::executeSQL(const std::string &sql, const std::vector<std::string> &params)
{
	if (AppState::extManager)
	{
		...
		auto res =
			AppState::extManager->dispatchOverride("dbExecute", {CppValue(CppValue::Type::String, sql), paramsVal});
		if (res.has_value())
		{
			return;
		}
...
ParallelQueryResult DatabaseManager::dbQueryNumber(int days, const std::string &mode, const std::string &sql,
												   const std::vector<std::string> &params)
{
	...
	{
		workers.emplace_back(
			[this, i, &fileNames, &path, activeFileName, &sql, &params, &localMutex, &rawRows]()
			{
				try
				{
					...
					else
					{
						...
						{
							std::cerr << "Historical file not found: " << fullPath << std::endl;
							Logger::log("[HPR] Historical file not found: " + fullPath.string());
							return;
						}
						...
						{
							std::cerr << "[DB QUERY PREPARE ERROR] " << sqlite3_errmsg(rawDb) << " | SQL: " << sql
									  << std::endl;
							Logger::log("[DB QUERY PREPARE ERROR] " + std::string(sqlite3_errmsg(rawDb)) +
										" | SQL: " + sql);
						}
					}
...
ParallelQueryResult DatabaseManager::dbQueryRange(std::string dateFrom, std::string dateTo, const std::string &mode,
			...
			[this, i, &fileNames, &path, activeFileName, &sql, &params, &localMutex, &rawRows]()
			{
				try
				{
					...
					else
					{
						...
						{
							std::cerr << "[DB QUERY PATH ERROR] " << sqlite3_errmsg(rawDb) << " | SQL: " << sql
									  << std::endl;
							Logger::log("[DB QUERY PATH ERROR] " + std::string(sqlite3_errmsg(rawDb)) +
										" | SQL: " + sql);
						}
					}
...
DatabaseManager::mergeQueryResults(const std::vector<std::map<std::string, std::string>> &rawRows,
...
std::vector<std::map<std::string, std::string>> DatabaseManager::querySQL(const std::string &sql,
																		  const std::vector<std::string> &params)
{
	if (AppState::extManager)
	{
		...
		auto res =
			AppState::extManager->dispatchOverride("dbQuery", {CppValue(CppValue::Type::String, sql), paramsVal});
		if (res.has_value())
		{
...
std::vector<std::map<std::string, std::string>> DatabaseManager::querySQL_Path(const std::string &dbPath,
																			   ...
																			   const std::vector<std::string> &params)
{
	if (AppState::extManager)
	{
		...
		auto res =
			AppState::extManager->dispatchOverride("dbQueryPath", {CppValue(CppValue::Type::String, dbPath),
