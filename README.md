...
HPR is also not a subscription service, not a SaaS dashboard, and not an Electron app. It is a compiled C++ binary. It starts in milliseconds and uses under 50MB of RAM while running on Linux, and under 10MB on Windows.
...
**Windows** sits around 8MB. That number is real and reflects only HPR's own code, data, and the minimal runtime libraries Windows loads.

**Linux** is more complicated. Tools like BTOP++, htop, and similar process monitors will report approximately 47MB. That number is not wrong, but it is misleading without context. Here is what it actually means.

HPR's own private memory on Linux -- heap, stack, all of your actual application data -- is roughly 22MB. The remaining ~25MB is shared GPU libraries that the OS maps into HPR's address space because Slint's renderer uses OpenGL for hardware-accelerated drawing. Specifically, the chain looks like this: Slint initializes OpenGL via EGL → the OS loads Mesa's Gallium driver for your GPU → Mesa's Gallium driver unconditionally loads libLLVM because it uses LLVM as a JIT compiler for shader code generation at the driver level. This happens even when all shaders are already compiled and cached. libLLVM is a 60MB+ library and its resident pages alone account for roughly 26MB of what process monitors report as HPR's memory.

This is not a memory leak. It is not HPR being inefficient. It is Mesa's architecture: any OpenGL application on Linux with a Mesa driver will pull in the same libLLVM chain. Your compositor, your browser, and every other GPU-accelerated application on the system already have these same pages loaded. Because they are shared, the kernel maps the same physical memory pages into each process. The proportional cost to HPR -- its actual share after dividing those shared pages across all processes using them -- is around 50MB PSS (Proportional Set Size), which is the most honest single number you can get from the kernel.
...
If you want to eliminate the GPU library overhead entirely, you can disable hardware acceleration in `config.csv`:

```csv
hardware-acceleration,false
```

This switches Slint to a CPU software renderer. The Mesa and libLLVM chain is never loaded, and total RSS drops to approximately 22MB. The UI remains fully functional and looks identical. The only tradeoff is slightly higher CPU usage during UI redraws -- for a background tracker that redraws every 500ms, this is completely imperceptible in practice.
...
**Language:** C++23
**UI:** Slint 1.16.1
**Database:** SQLite3 (bundled amalgamation) via sqlite_modern_cpp# HPR - Human Pattern Recorder
