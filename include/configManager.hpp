...
class ConfigManager
{
  public: // No need for destructor
	...
	template <typename T> T getConfig(const std::string requestedParam, const T &defaultValue);

	template <typename T> void setConfig(const std::string paramName, const T &value);
